...
public class AdvancedWebsocketStream
{
    ...
    public async Task SendPacket(object packet)
    {
        ...
        // Send length
        var length = packetBuffer.Length;
        var lengthBuffer = BitConverter.GetBytes(length);

        var buffer = lengthBuffer.Concat(packetBuffer).ToArray();
    ...
    private byte[] EncodePacket(object packet)
    {
        ...
        var result = headerBuffer.Concat(bodyBuffer).ToArray();
        
        Logger.Info($"Header: {headerBuffer.Length}, Body: {bodyBuffer.Length}, Result: {result.Length}, Text: {jsonText}");
